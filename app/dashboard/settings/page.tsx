...
export default function SettingsPage() {
  ...
  const handleAddRole = async (e: React.FormEvent) => {
    ...
    const newRole = await api.roles.create(organizationId, { name: newRoleData.role, description: newRoleData.description, branchId: selectedBranchId });
  ...
  const loadRoles = useCallback(async () => {
    ...
    const [rolesResponse, memberResponse, catalogueResponse] = await Promise.all([
      api.roles.listOrganization(organizationId, selectedBranchId),
      api.organization.getMembers(organizationId, { limit: 200 }),
      api.permissions.listCatalogue(selectedBranchId),
    ]);
  ...
  useEffect(() => {
    ...
    setPermissionScope(selectedBranchId ?? "org_wide");
  }, [selectedBranchId]);
  ...
  return (
    <div className="max-w-7xl mx-auto pb-12">
      ...
      {planPickerOpen && <ModalLayer className="flex items-center justify-center bg-black/40 p-4">
        <div className="max-h-[calc(100dvh-32px)] w-full max-w-[680px] overflow-y-auto rounded-[12px] border border-grey-4 bg-white p-5 shadow-[0px_16px_32px_-16px_rgba(0,0,0,0.24)]">
        ...
        {isEditPermissionsModalOpen && selectedRoleForEdit && (
          <DrawerLayer onClose={() => setIsEditPermissionsModalOpen(false)} label="Manage permissions">
            <div className="flex h-full w-full flex-col overflow-hidden bg-white">
              ...
              <div className="min-h-0 flex-1 overflow-y-auto p-6 overscroll-contain">
                <div className="flex flex-wrap gap-2">
              ...
              <div className="flex shrink-0 justify-end gap-3 border-t border-grey-4 bg-grey-5/50 p-6">
