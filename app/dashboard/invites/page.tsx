...
export default function PendingInvitesPage() {
  ...
  return <div className="mx-auto flex max-w-7xl flex-col gap-5 pb-12">
    ...
    <section className="rounded-xl border border-grey-4 bg-white p-6">
      ...
      {loading ? <p className="py-12 text-center text-sm text-grey-3">Loading pending invitations…</p> : filtered.length === 0 ? <div className="py-12 text-center"><MailPlus className="mx-auto mb-3 h-8 w-8 text-grey-3" /><p className="text-sm text-grey-3">No pending invitations in this scope.</p></div> : <div className="overflow-x-auto"><table className="w-full text-left"><thead><tr className="border-b border-grey-4 text-xs uppercase text-grey-2"><th className="px-4 py-3">Email</th><th className="px-4 py-3">Branch</th><th className="px-4 py-3">Sent</th><th className="px-4 py-3">Expires</th><th className="px-4 py-3 text-right">Action</th></tr></thead><tbody>{filtered.map((invite) => <tr key={invite.inviteId} className="border-b border-grey-4"><td className="px-4 py-3 text-sm font-medium text-grey-1">{invite.invitedEmail}</td><td className="px-4 py-3 text-sm text-grey-2">{invite.branchId ? branchNames.get(invite.branchId) ?? "Assigned branch" : "Organization-wide"}</td><td className="px-4 py-3 text-sm text-grey-2">{new Date(invite.createdAt).toLocaleDateString()}</td><td className="px-4 py-3 text-sm text-grey-2">{new Date(invite.expiresAt).toLocaleDateString()}</td><td className="px-4 py-3 text-right"><button onClick={() => setInviteToCancel(invite)} className="inline-flex items-center gap-1 rounded-lg px-3 py-2 text-xs font-medium text-red-600 hover:bg-red-50"><Trash2 className="h-4 w-4" /> Cancel</button></td></tr>)}</tbody></table></div>}
    </section>
