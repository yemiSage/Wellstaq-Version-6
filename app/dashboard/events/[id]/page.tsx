...
import type { EventItem, EventParticipantInfo, MessageResponse, Member } from "@/types/api";

export default function EventDetailPage({ params }: { params: Promise<{ id: string }> }) {
  const { organizationId, currentUser, members: ORG_MEMBERS } = useDashboardData();
  ...
  const filteredMembers = ORG_MEMBERS.filter(
    (m: Member) =>
      ...
      >
        <div className="space-y-4">
          ...
          <div className="max-h-[300px] space-y-2 overflow-y-auto pr-2 no-scrollbar">
            {filteredMembers.map((emp: Member) => (
              <button key={emp.id} type="button" onClick={() => toggleMemberSelection(emp.id)} className={`flex w-full items-center justify-between rounded-[8px] border p-3 text-left transition-all ${selectedMemberIds.includes(emp.id) ? "border-primary-1 bg-primary-1/5" : "border-grey-4 hover:border-grey-3"}`}>
                <span className="flex items-center gap-3">
                  <span className="relative h-10 w-10 overflow-hidden rounded-full bg-grey-4"><Image src={emp.avatar} alt={emp.name} fill className="object-cover" /></span>
