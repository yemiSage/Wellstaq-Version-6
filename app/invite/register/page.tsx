...
function InviteRegistrationContent() {
  ...
  const verifyCode = async () => {
    ...
    catch (reason) { setError(reason instanceof Error ? reason.message : "The verification code is incorrect or expired."); }
    finally { setLoading(false); }
  };
  ...
  const register = async () => {
    ...
    try {
      ...
      if (!response.accessToken || !response.refreshToken || !response.tokenType) throw new Error("Registration completed but no login session was returned.");
      setAuthTokens({ accessToken: response.accessToken, refreshToken: response.refreshToken, tokenType: response.tokenType });
      router.replace("/dashboard");
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Unable to complete registration."); }
    finally { setLoading(false); }
  };
  ...
  return <SplitLayout><OnboardingPane><div className="mx-auto flex w-full max-w-xl flex-1 flex-col pb-10">
    ...
    {step === "profile" && <div className="space-y-6"><div><h1 className="text-2xl font-bold text-grey-1">Create your account</h1><p className="mt-2 text-sm text-grey-2">Complete your profile and wellness baseline.</p></div>
      ...
      <div><h2 className="mb-3 text-sm font-bold text-grey-1">Wellness baseline</h2><div className="space-y-3">{dimensions.map((dimension) => <div key={dimension} className="grid gap-2 rounded-xl border border-grey-4 p-3 sm:grid-cols-3"><span className="text-sm font-medium text-grey-1">{label(dimension)}</span><select value={baseline[dimension].level} onChange={(e) => setBaseline({ ...baseline, [dimension]: { ...baseline[dimension], level: e.target.value } })} className="h-9 rounded-lg border border-grey-4 px-2 text-sm">{levels.map((item) => <option key={item} value={item}>{label(item)}</option>)}</select><select value={baseline[dimension].reason} onChange={(e) => setBaseline({ ...baseline, [dimension]: { ...baseline[dimension], reason: e.target.value } })} className="h-9 rounded-lg border border-grey-4 px-2 text-sm">{reasons.map((item) => <option key={item} value={item}>{label(item)}</option>)}</select></div>)}</div></div>
      <div><h2 className="mb-2 text-sm font-bold text-grey-1">Your priorities</h2><div className="flex flex-wrap gap-2">{priorities.map((priority) => { const selected = selectedPriorities.includes(priority); return <button type="button" key={priority} onClick={() => setSelectedPriorities(selected ? selectedPriorities.filter((item) => item !== priority) : [...selectedPriorities, priority])} className={`rounded-full border px-3 py-2 text-xs ${selected ? "border-primary-1 bg-primary-1/5 text-primary-1" : "border-grey-4 text-grey-2"}`}>{label(priority)}</button>; })}</div></div>
