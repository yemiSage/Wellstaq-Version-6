...
function PasswordField({ label: fieldLabel, value, visible, onToggle, onChange }: { label: string; value: string; visible: boolean; onToggle: () => void; onChange: (value: string) => void }) {
  return <Field label={fieldLabel}><div className="relative"><Input type={visible ? "text" : "password"} autoComplete="new-password" value={value} onChange={(event) => onChange(event.target.value)} className="pr-12" /><button type="button" onClick={onToggle} aria-label={visible ? `Hide ${fieldLabel.toLowerCase()}` : `Show ${fieldLabel.toLowerCase()}`} className="absolute right-3 top-1/2 -translate-y-1/2 rounded p-1 text-grey-3 hover:text-grey-1">{visible ? <EyeOff className="h-5 w-5" /> : <Eye className="h-5 w-5" />}</button></div></Field>;
}
  ...
  return <SplitLayout><OnboardingPane><div className="mx-auto flex w-full max-w-xl flex-1 flex-col pb-10">
    ...
    {step === "profile" && <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <Field label="Country"><select value={form.countryCode} onChange={(event) => { const country = countries.find((item) => item.isoCode === event.target.value); setForm({ ...form, countryCode: event.target.value, country: country?.name ?? "", stateCode: "", state: "" }); }} className="h-12 w-full border border-grey-4 bg-white text-sm"><option value="">Select country</option>{countries.map((country) => <option key={country.isoCode} value={country.isoCode}>{country.name}</option>)}</select></Field>
        <Field label="State"><select value={form.stateCode} disabled={!form.countryCode} onChange={(event) => { const state = states.find((item) => item.isoCode === event.target.value); setForm({ ...form, stateCode: event.target.value, state: state?.name ?? "" }); }} className="h-12 w-full border border-grey-4 bg-white text-sm disabled:bg-grey-5"><option value="">Select state</option>{states.map((state) => <option key={state.isoCode} value={state.isoCode}>{state.name}</option>)}</select></Field>
