...
export default function SettingsPage() {
  ...
  const cannotEditRole = (role: RoleItem) => {
    const protectedTargets: Record<string, string[]> = {
      admin: ["super_admin", "admin", "branch_manager"],
      branch_manager: ["super_admin", "branch_manager"],
      team_leader: ["super_admin", "admin", "branch_manager", "team_leader"],
      member: ["super_admin", "admin", "branch_manager", "team_leader", "member"],
      employee: ["super_admin", "admin", "branch_manager", "team_leader", "member"],
    };
    return role.name === "super_admin"
      || (currentUser?.role === "branch_manager" && role.name === "admin" && !role.branchId)
      || (protectedTargets[currentUser?.role ?? "member"] ?? []).includes(role.name);
  };

  const handleAddRole = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!organizationId) return;
    const branchId = branches.find((branch) => branch.name === activeBranch)?.id;
    const newRole = await api.roles.create(organizationId, { name: newRoleData.role, description: newRoleData.description, branchId });
