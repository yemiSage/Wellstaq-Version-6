                ...
                {rolesList.map((role) => (
                  <div key={role.id} className="p-5 rounded-xl border border-grey-4 hover:border-primary-1/30 transition-all">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {roleBundleNames(role).map((permission) => <span key={permission} className="px-2 py-1 bg-primary-1/5 text-primary-1 text-[10px] font-medium rounded-md border border-primary-1/10">{permission}</span>)}
                    ...
                    <div className="flex items-center justify-end gap-3 mt-4 pt-4 border-t border-grey-4/50">
                      <button
                        disabled={role.isSystem || role.isDefault || !canAssignRoles}
                        onClick={() => void handleDeleteRole(role)}
                        className={`text-xs font-bold ${role.isSystem || role.isDefault || !canAssignRoles ? "text-grey-4 cursor-not-allowed" : "text-red-500 hover:text-red-600"}`}
                      >
                        Delete Role
                      </button>
                      <button
                        disabled={!role.isSystem && (!canGrantPermissions || !canRevokePermissions)}
                        onClick={() => {
            ...
            >
              <div className="p-6 border-b border-grey-4 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-grey-2">{selectedRoleForEdit.isSystem ? "System-role permissions are view only. Copy the role to customize it." : "Select the permission groups assigned to this organization role."}</p>
                </div>
                <button
                  onClick={() => setIsEditPermissionsModalOpen(false)}
              ...
              <div className="p-6">
                <div className="flex flex-wrap gap-2">
                  {Object.entries(permissionBundles).map(([label, names]) => {
                    ...
                    return <button key={label} disabled={selectedRoleForEdit.isSystem} onClick={() => toggleRoleBundle(label)} className={`px-3 py-1.5 rounded-full text-xs font-medium transition-all border ${isSelected ? "bg-white border-primary-1 text-primary-1 shadow-sm" : "bg-white border-grey-4 text-grey-2 hover:border-grey-3"} disabled:cursor-default`}>{label}</button>;
                  })}
                </div>
              </div>
