...
export default function SettingsPage() {
  ...
  const { user, updateUser, organizationId, currentUser, branches, activeBranch } = useDashboardData();
  ...
  const loadRoles = useCallback(async () => {
    ...
    const [rolesResponse, memberResponse, catalogueResponse] = await Promise.all([
      api.roles.listOrganization(organizationId, branches.find((branch) => branch.name === activeBranch)?.id),
  ...
  }, [activeBranch, branches, organizationId]);
                ...
                {rolesList.map((role) => (
                  <div key={role.id} className="p-5 rounded-xl border border-grey-4 hover:border-primary-1/30 transition-all">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {(role.permissions ?? []).map((permission) => <span key={permission} className="px-2 py-1 bg-primary-1/5 text-primary-1 text-[10px] font-medium rounded-md border border-primary-1/10">{readablePermission(permission)} · {role.branchId ? "This branch" : role.permissionScopes?.[permission] === "org" ? "Organization-wide" : "Assigned branches"}</span>)}
