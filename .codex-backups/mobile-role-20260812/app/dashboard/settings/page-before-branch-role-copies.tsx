...
export default function SettingsPage() {
  ...
  const canRevokeRoles = currentUser ? hasPermission(currentUser.permissions, "member.role.revoke") : false;
  ...
  const permissionBundles: Record<string, string[]> = {
    "Manage Branches": ["branch.create", "branch.update", "branch.delete", "branch.manager.assign"],
    "Manage Members": ["member.invite", "member.remove", "member.suspend", "member.reinstate", "member.role.assign", "member.role.revoke", "member.department.assign", "member.branch.assign"],
    "Manage Events & Challenges": ["event.create", "event.update", "event.delete", "challenge.create", "challenge.update", "challenge.delete"],
    "Manage Posts & Community": ["post.delete", "post.moderate", "post.view_all", "story.delete", "comment.delete", "chat.moderate", "club.create", "club.update", "club.delete", "club.member.remove"],
    "View & Export Wellbeing": ["overview", "org.stats", "wellbeing.view_team", "wellbeing.export"],
    "Manage Permissions": ["member.permission.grant", "member.permission.revoke"],
    "Manage Organization Settings": ["org.settings.update"],
    "Manage Billing": ["org.billing.manage"],
    "Deactivate Organization": ["org.deactivate"],
  };

  const roleBundleNames = (role: RoleItem) => Object.entries(permissionBundles)
    .filter(([, names]) => names.some((name) => (role.permissions ?? []).includes(name)))
    .map(([label]) => label);
  const bundleScope = (role: RoleItem, label: string) => {
    const scopes = new Set(permissionBundles[label].filter((name) => (role.permissions ?? []).includes(name)).map((name) => role.permissionScopes?.[name]));
    if (scopes.size > 1) return "Mixed scope";
    return scopes.has("org") ? "Organization-wide" : "Assigned branches";
  };

  const handleAddRole = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!organizationId) return;
    const newRole = await api.roles.create(organizationId, { name: newRoleData.role, description: newRoleData.description });
    setRolesList([...rolesList, newRole]);
    setIsAddRoleModalOpen(false);
    setNewRoleData({ role: "", description: "" });
  ...
  const toggleRoleBundle = (label: string) => {
    if (!selectedRoleForEdit || selectedRoleForEdit.isSystem) return;
    const bundle = permissionBundles[label];
    const current = new Set(selectedRoleForEdit.permissions ?? []);
    const selected = bundle.every((name) => current.has(name));
    bundle.forEach((name) => selected ? current.delete(name) : current.add(name));
    setSelectedRoleForEdit({ ...selectedRoleForEdit, permissions: [...current] });
  };

  const saveRolePermissions = async () => {
    if (!organizationId || !selectedRoleForEdit || selectedRoleForEdit.isSystem) return;
    const updated = await api.roles.updatePermissions(organizationId, selectedRoleForEdit.id, selectedRoleForEdit.permissions ?? []);
    setRolesList((roles) => roles.map((role) => role.id === updated.id ? updated : role));
                ...
                {rolesList.map((role) => (
                  <div key={role.id} className="p-5 rounded-xl border border-grey-4 hover:border-primary-1/30 transition-all">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {roleBundleNames(role).map((permission) => <span key={permission} className="px-2 py-1 bg-primary-1/5 text-primary-1 text-[10px] font-medium rounded-md border border-primary-1/10">{permission} · {bundleScope(role, permission)}</span>)}
                    ...
                    <div className="flex items-center justify-end gap-3 mt-4 pt-4 border-t border-grey-4/50">
                      ...
                      <button
                        disabled={role.name === "super_admin" || (!canGrantPermissions && !canRevokePermissions)}
                        onClick={() => {
                      ...
                      >
                        {role.name === "super_admin" ? "Protected" : "Edit Permissions"}
                      </button>
                    </div>
                  </div>
                ))}
            ...
            >
              <div className="p-6 border-b border-grey-4 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-grey-2">Select the permission groups assigned to this organization role. Organization-wide applies to every branch; Assigned branches applies to each branch the user manages, or their home branch by default.</p>
                </div>
                <button
                  onClick={() => setIsEditPermissionsModalOpen(false)}
              ...
              <div className="p-6">
                <div className="flex flex-wrap gap-2">
                  {Object.entries(permissionBundles).map(([label, names]) => {
                    const isSelected = names.every((name) => (selectedRoleForEdit.permissions ?? []).includes(name));
                    return <button key={label} disabled={selectedRoleForEdit.name === "super_admin"} onClick={() => toggleRoleBundle(label)} className={`px-3 py-1.5 rounded-full text-xs font-medium transition-all border ${isSelected ? "bg-white border-primary-1 text-primary-1 shadow-sm" : "bg-white border-grey-4 text-grey-2 hover:border-grey-3"} disabled:cursor-default`}>{label}{isSelected ? ` · ${bundleScope(selectedRoleForEdit, label)}` : ""}</button>;
                  })}
                </div>
              </div>
