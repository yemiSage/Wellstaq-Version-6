  ...
  const confirmMemberChange = async () => {
    ...
    try {
      if (pendingChange === "role") {
        ...
        const memberRole = manageRoles.find((item) => item.name === "member");
        await api.roles.revokeFromMember(organizationId, selectedMember.id, permissionBranchId);
        setMembers((items) => items.map((item) => item.id === selectedMember.id ? { ...item, roleId: memberRole?.id ?? null, roleName: "member" } : item));
        setSelectedMember({ ...selectedMember, roleId: memberRole?.id ?? null, roleName: "member" });
        setMemberChanges((current) => ({ ...current, roleId: memberRole?.id ?? "" }));
